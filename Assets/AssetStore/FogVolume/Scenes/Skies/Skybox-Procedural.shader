Shader "Fog Volume/Skybox" {
Properties {
	[hideininspector][KeywordEnum(None, Miranda, High Quality)] _SunDisk ("Sun", Int) = 1
	_SunSize ("Sun Size", Range(0,1)) = 0.04
	_SunDiscEdges("Sun Disk Edges", Range(.5,3)) = 1
	_SunIntensity("Sun Intensity", Range(0,1000)) = 40
...
SubShader {
	...
	Pass {
		...
		uniform half _SunIntensity, _SunDiscEdges;
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			#endif
				mie = pow(mie,_SunDiscEdges);
				mie = min(_SunIntensity, mie);
				col += mie * IN.sunColor;
			}
		#endif
